public class MobilePhone {
    ...
    public void call(float minutes, char network) {
        float cost = network == this.network ? this.ratePerCall: this.ratePerCall * 2;
        this.load -= minutes * cost;
    }

    public void text(int length, char network) {
        float cost = network == this.network ? this.ratePerText: this.ratePerText * 2;
        this.load -= ((float) length / MobilePhone.maxTextSize) * cost;
    }
